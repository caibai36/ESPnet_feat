...
# cp -r ../asr1/{cmd.sh,data,local,RESULTS.md,conf,path.sh,run.sh} . # basic structure
# exp_lm_dir=../asr1/exp/train_rnnlm_pytorch_lm_word65000; if [ -d $exp_lm_dir ]; then mkdir -p exp; cd exp; ln -sf ../$exp_lm_dir $(basename $exp_lm_dir); cd .. ; fi # lm
# concat_dir=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/s0/exp/concatfeat2asr; if [ -d $exp_lm_dir ]; then mkdir -p exp; cd exp; ln -sf $concat_dir concatfeat2asr; cd .. ; fi # feat
...
# feature for training, development, and test sets.
train_feat=exp/concatfeat2asr/asr_wsj1_concat_MFCC40_HybridMSE_wsj1_seed123nseed2022K98l8r8_bs256_hd512_nl5_ne20/data/train/feats.scp
dev_feat=exp/concatfeat2asr/asr_wsj1_concat_MFCC40_HybridMSE_wsj1_seed123nseed2022K98l8r8_bs256_hd512_nl5_ne20/data/dev/feats.scp
test_feat=exp/concatfeat2asr/asr_wsj1_concat_MFCC40_HybridMSE_wsj1_seed123nseed2022K98l8r8_bs256_hd512_nl5_ne20/data/test/feats.scp
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    data2json.sh --feat ${train_feat} --nlsyms ${nlsyms} \
         data/${train_set} ${dict} > ${feat_tr_dir}/data.json
    data2json.sh --feat ${dev_feat} --nlsyms ${nlsyms} \
         data/${train_dev} ${dict} > ${feat_dt_dir}/data.json
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
	mkdir -p ${feat_recog_dir}

	if [ $rtask = "test_dev93" ]; then rfeat=${dev_feat}; fi
	if [ $rtask = "test_eval92" ]; then rfeat=${test_feat}; fi
        data2json.sh --feat ${rfeat} \
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 3 ] && ! ${skip_lm_training}; then
